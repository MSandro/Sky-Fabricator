...
public class KilnBlockEntity extends LootableContainerBlockEntity {
    ...
    public KilnBlockEntity(BlockPos pos, BlockState state) {
        super(ModRegistry.KILN_ENTITY_TYPE, pos, state);
            ...
            if (out.isEmpty()) {
                if (!crucible.isEmpty()) {
                    if (!item.isEmpty() && item.getItem() == Items.COBBLESTONE && item.getCount() >= COBBLESTONE_COST) {
                        cook(item, fuel, crucible, COBBLESTONE_COOK_TIME, COBBLESTONE_COST, ModRegistry.LAVA_CRUCIBLE);
                    } else {
                        this.cook_time = 0;
                        this.progress = 0;
                    }
                } else {
                    if (!item.isEmpty() && item.getItem() == ModRegistry.RAW_CRUCIBLE) {
                        cook(item, fuel, crucible, RAW_CRUCIBLE_COOK_TIME, 1, ModRegistry.CRUCIBLE);
                    } else {
                        this.cook_time = 0;
                        this.progress = 0;
                    }
